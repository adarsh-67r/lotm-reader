...
async def process_file(index, file, semaphore, attributes):
    ...
    with open(f"./build3/{file}", "r", encoding="utf-8") as f:
        htmlData = "\n".join(f.readlines()[0:11])
        match = re.search(r"Chapter\s+(\d+)(?=[:\s<])", htmlData)
        if match:
            index = int(match.group(1))
        else:
            index += 2000

    if Path(f"./chapters/lotm/fantl/{index:04d}.md").exists():
        print(
            f"file {index:04d}.md already exits [{index}] [{originalIndex}] [{file}] [{match.group(1)}]"
        )

    async with semaphore:
        output_path = f"./chapters/lotm/fantl/{index:04d}.md"
        source_path = f"./build3/{file}"

        # Async Pandoc Call
        proc = await asyncio.create_subprocess_exec(
            "pandoc",
            "-f",
            "html-native_divs-auto_identifiers+smart",
            "-t",
            "markdown",
            "-o",
            output_path,
        ...
        # Reading file (using to_thread to keep it async-compatible)
        def read_and_clean():
            with open(output_path, "r", encoding="utf-8") as f:
                content = f.read()

            # --- YOUR ORIGINAL REGEX BLOCK ---
            content = re.sub(
                r"\[([^\]]+)\]\{[^{}]*\.run_1[^{}]*\}",
                r"***\1***",
                content,
...
async def main():
    ...
    files = os.listdir("./build3")

    shutil.rmtree("./chapters/lotm/fantl")
    os.mkdir("./chapters/lotm/fantl")
    attributes = []
